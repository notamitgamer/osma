...
def build_db_from_csv_file(csv_path: str):
    ...
    with open(csv_path, encoding="utf-8", errors="replace", newline="") as f:
        ...
        for row in reader:
            ...
            if len(batch) >= 10_000:
                conn.executemany(
                    "INSERT OR REPLACE INTO pypi_packages VALUES (?, ?)", batch
                )
                conn.commit()
                batch = []
                if count % 500_000 == 0:
...
def browse(
    ...
    conn   = get_conn()
    rows   = conn.execute(
        "SELECT package_no, package_name FROM pypi_packages ORDER BY package_no LIMIT ? OFFSET ?",
        (limit, offset)
    ).fetchall()
    conn.close()
    return {
...
def search(
    ...
    exact = conn.execute(
        "SELECT package_no, package_name, 0 AS rank FROM pypi_packages WHERE LOWER(package_name) = ? LIMIT 1",
        (q_lower,)
    ).fetchall()

    starts = conn.execute(
        """SELECT package_no, package_name, 1 AS rank FROM pypi_packages
    ...
    if remaining > 0:
        ...
        raw_contains = conn.execute(
            """SELECT package_no, package_name, 2 AS rank FROM pypi_packages
