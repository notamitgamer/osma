...
Serves search and browse queries over 793K+ python packages stored in SQLite.
...
| `GET /search?q=grok&limit=250` | Ranked search — exact → starts-with → contains |
...
On first startup the server downloads the pypi dataset from
`notamitgamer/npypi` on HuggingFace and builds a SQLite index.
